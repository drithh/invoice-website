    ...
    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
      ...
      <div>
        <span class="relative z-0 inline-flex rounded-md shadow-sm">
          ...
          @else
            <button data-url="{{ $paginator->previousPageUrl() }}" onclick="fetchData(this.getAttribute('data-url'))"
              rel="prev"
          ...
          @foreach ($elements as $element)
            ...
            @if (is_array($element))
              @foreach ($element as $page => $url)
                ...
                @else
                  <button data-url="{{ $url }}" onclick="fetchData(this.getAttribute('data-url'))"
                    class="relative -ml-px inline-flex cursor-pointer items-center border border-gray-300 bg-white px-4 py-2 text-sm font-medium leading-5 text-gray-700 ring-gray-300 transition duration-150 ease-in-out hover:text-gray-500 focus:z-10 focus:border-blue-300 focus:outline-none focus:ring active:bg-gray-100 active:text-gray-700"
          ...
          {{-- Next Page Link --}}
          @if ($paginator->hasMorePages())
            <button data-url="{{ $paginator->nextPageUrl() }}" onclick="fetchData(this.getAttribute('data-url'))"
              rel="next"
